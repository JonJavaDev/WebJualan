(() => {
  ...
  const openCleanupConfirmModal = () => {
    if (!cleanupConfirmModal) {
      return;
    }
    cleanupConfirmModal.classList.remove("hidden");
    cleanupConfirmModal.setAttribute("aria-hidden", "false");
  };

  const closeCleanupConfirmModal = () => {
    if (!cleanupConfirmModal) {
      return;
    }
    cleanupConfirmModal.classList.add("hidden");
    cleanupConfirmModal.setAttribute("aria-hidden", "true");
  };
  ...
  const performCleanup = async () => {
    ...
    try {
      if (cleanupConfirmBtn) {
        ...
        cleanupConfirmBtn.textContent = "Menghapus...";
      }

      const data = await apiFetch("/admin/cleanup-test-orders", {
        method: "POST",
        headers: getHeaders(),
      });

      closeCleanupConfirmModal();
      setMessage(`Pembersihan selesai. ${data.deleted || 0} pesanan dihapus, ${data.remaining || 0} tersisa.`, "success");
      await loadOrders(false);
      await loadPreorders(false);
    } catch (error) {
      setMessage(error.message || "Gagal membersihkan data.", "error");
    } finally {
      if (cleanupConfirmBtn) {
        cleanupConfirmBtn.disabled = false;
        cleanupConfirmBtn.textContent = "Hapus";
      }
    }
  };
  ...
  if (cleanupConfirmModal) {
    cleanupConfirmModal.addEventListener("click", (event) => {
      if (event.target === cleanupConfirmModal) {
        closeCleanupConfirmModal();
      }
    });
  }
